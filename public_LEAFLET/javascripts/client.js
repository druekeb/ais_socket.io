$(document).ready(function() {
     ...
     // Websocket
      ...
      var map = L.map('map').setView([53.5455,9.95], 15);
      ...
      function changeRegistration()
      {
        ...
        console.debug("register for zoomLevel "+map.getZoom());
    ...
    function paintToMap(v){
      if(v.pos != null)
      {
        ...
        v.angle = calcAngle(v);
        //Marker für BaseStations, Helicopter, Seezeichen
        ...
        else //Behandlung schiffsbezogener messages (1,2,3,5)
        {
          var vectorPoints = [ new L.LatLng(v.pos[1],v.pos[0])];
          //SpeedVektoren für Schiffe, die sich mit mind. 3 Knoten bewegen
          ...
          //   vectorPoints = chunk(vectorPoints, 1);
          // }
          //Marker für alle Schiffe
         // v.marker = L.animatedMarker(vectorPoints,{
          ...
          {
           var polygonPoints = calcPolygonPoints(v);
            v.polygon = new L.animatedPolygon(polygonPoints,{sog:v.sog, angle:v.angle, zoom:map.getZoom()});
    ...
    //ermittle aus den Abmessungen eines Schiffes dessen 5 Polygonpunkte
    function calcPolygonPoints(vessel) {
  ...
  function chooseIcon(obj){
     ...
     function createVectorPoints(vessel) {
       ...
       var targetPoint = calcVector(lon, lat, (vectorLength/10000) , sin_angle, cos_angle);
       vectorPoints.push(targetPoint);
       return vectorPoints;
   }
  ...
  function calcVector(lon, lat, sog, sin, cos){
    var dy_deg = -(sog * cos);
    var dx_deg = -(- sog * sin)/(Math.cos((lat)*(Math.PI/180.0)));
